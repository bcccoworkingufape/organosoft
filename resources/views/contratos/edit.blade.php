<x-auth>
    ...
    <x-slot name="bg_main">
        <form id="form" action="{{route('contratos.update', $contrato)}}" method="POST" enctype="multipart/form-data" class="justify-center flex">
            ...
            <x-form class="pt-4 w-2/3 flex flex-wrap" x-data="{status: '{{old('status', $contrato->status)}}'}">
                ...
                <x-form-control class="w-1/2 pr-1">
                    ...
                    <x-datepicker id="inicio" type="date" name="inicio" value="{{old('inicio', $contrato->inicio)}}"/>
                </x-form-control>
                <x-form-control class="w-1/2 pl-1">
                    <x-label for="fim" value="Data final:" />
                    <x-datepicker id="fim" type="date" name="fim" value="{{old('fim', $contrato->fim)}}"/>
                </x-form-control>
                <x-form-control class="w-full">
                ...
                <x-form-control x-bind:class="status == 'outro' ? 'w-1/2 pr-1' : 'w-full'">
                    ...
                    <x-select id="status" x-model="status" name="status" required>
                        ...
                        <option @selected(old('status', $contrato->status)) value="construcao">Em construção</option>
                        <option @selected(old('status', $contrato->status)) value="execucao">Em execução</option>
        ...
        <div class="flex justify-around mt-4">
            <a href="{{route('produtores.show', $contrato->produtor)}}" class="bg-red-600 hover:bg-red-700 organosoft-btn">
                Cancelar
            </a>
            <x-button form="form">
                Atualizar
