<div>
    @switch($tipo)
        @case('icon')
            <a wire:click="toggleModal" href="#" title="Deletar categoria veiculo">
                @switch($cor)
                    @case('white')
    ...
    <x-jet-confirmation-modal wire:model="show">
        ...
        <x-slot name="content">Tem certeja que deseja deletar essa categoria de veículos?</x-slot>
        <x-slot name="footer">
            ...
            <x-button class="ml-2" wire:click.prevent="deletar" wire:loading.attr="disabled">
                Deletar deletar categoria de veículo
            </x-button>
        </x-slot>
